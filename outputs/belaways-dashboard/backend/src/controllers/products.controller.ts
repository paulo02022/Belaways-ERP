...
export const productsController = {
  index: asyncHandler(async (request, response) => {
    ...
    const products = await tinyService.listProducts({ search: query.search, page: query.page });
    const filtered = products.filter((product) => {
      if (query.category && product.category !== query.category) return false;
      if (query.stock === 'low') return product.stock > 0 && product.stock <= product.minimumStock;
      if (query.stock === 'out') return product.stock <= 0;
      return true;
    });
  ...
  show: asyncHandler(async (request, response) => {
    ...
    const product = await tinyService.getProduct(id);
