...
export const Login = () => {
  ...
  const { signIn, enterPreview, user, isPreview } = useAuth();
    ...
    <main className="grid min-h-screen bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-white lg:grid-cols-[1.05fr_0.95fr]">
      <section className="flex min-h-screen items-center justify-center px-6 py-10">
        <div className="w-full max-w-md">
          ...
          {isPreview ? (
            <Button variant="secondary" className="mt-3 w-full" onClick={handlePreview}>
              Abrir prévia local
            </Button>
          ) : null}
        </div>
      ...
      <section className="hidden min-h-screen border-l border-zinc-200 bg-white p-10 dark:border-zinc-800 dark:bg-zinc-900 lg:flex lg:flex-col lg:justify-between">
        <div className="grid grid-cols-2 gap-4">
          {[
            ['Produtos monitorados', '12.840'],
            ['Alertas críticos', '18'],
            ['Pedidos no mês', '3.216'],
            ['SLA logístico', '96,8%'],
          ].map(([label, value]) => (
            <div key={label} className="rounded-lg border border-zinc-200 p-5 dark:border-zinc-800">
